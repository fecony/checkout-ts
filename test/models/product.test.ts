...
describe('product', () => {
  ...
  describe('price', () => {
    ...
    it('should maintain the accuracy of the conversion', () => {
      ...
      expect(product.price).toBeCloseTo(25.0, 2)
    ...
    describe('zero price', () => {
      it('should allow setting price to zero', () => {
        const product = new Product('000', 'Sample Product', 0.0)
        expect(product.price).toBeCloseTo(0.0, 2)
    ...
    describe('price rounding', () => {
      it('should round price and priceInCents correctly', () => {
        ...
        expect(product.price).toBeCloseTo(3.0, 2)
      ...
      it('should round price with many decimal places correctly', () => {
        const product = new Product('000', 'Sample Product', 3.1415926535)
  ...
  describe('negative price', () => {
    it('should throw NegativePriceError when price is negative', () => {
      expect(() => {
        const _product = new Product('000', 'Sample Product', -1.0)
      }).toThrow(NegativePriceError)
    })

    it('should throw NegativePriceError when setting negative price', () => {
      const product = new Product('000', 'Sample Product', 2.0)
      expect(() => {
        product.price = -3.0
      }).toThrow(NegativePriceError)
    })
